...
Minecraft Mod JAR 多语言本地化辅助工具，面向 Minecraft 玩家、整合包作者、服务器维护者和模组翻译维护者。

项目会扫描 Mod JAR 中的源语言文件，并为目标 Minecraft 语言代码生成资源包 ZIP 和 HTML 报告。默认目标语言是 `zh_cn`，也支持选择 Minecraft 官方语言列表中的一百多种 locale；项目同时提供 Web UI、桌面版、翻译记忆、术语表、硬编码文本扫描和补丁 Mod 模板，帮助维护者把重复、容易出错的本地化流程自动化。

English summary: `mc-mod-i18n` is an open-source localization assistant for Minecraft mods. It generates localized resource packs from mod JARs for Minecraft language locales, reports untranslated or risky strings, supports multiple translation providers, and helps players and pack maintainers localize modded Minecraft content more reliably.
...
- 为不同语言社区快速生成可用的目标语言资源包，例如 `zh_cn`、`ja_jp`、`fr_fr`、`de_de` 等 Minecraft locale。
